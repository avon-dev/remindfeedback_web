...
export default (state = initialState, action) => {
  switch (action.type) {
    ...
    case FEEDBACK_SUB_ADD_SUCCESS:
      const addedData = action.data.success
        ? action.data.data[action.data.data.length - 1]
        : "";
      return {
        ...state,
        ...
        subject: action.data.success
          ? [...state.subject, addedData]
          : state.subject
      };
    ...
    case FEEDBACK_SUB_UPDATE_SUCCESS:
      ...
      if (action.data.success) {
        const subjectIndex = state.subject.findIndex(
          v => v.category_id === action.data.data.category_id
        );
    ...
    case FEEDBACK_SUB_DELETE_SUCCESS:
      const index = state.subject.findIndex(
        v => parseInt(v.category_id) === parseInt(action.data.data)
      );
