...
const Subjects = () => {
  ...
  const {
    ...
    message,
    isUpdatedSubject
  } = useSelector(state => state.feedbackSubject);
  ...
  const data = subject.map((data, index) => {
    return {
      ...
      update: [
        { category_title: data.category_title, category_id: data.category_id }
      ]
    };
  });
  ...
  const PopupUpdateSubject = e => {
    ...
    const index = subject.findIndex(
      v => parseInt(v.category_id) === parseInt(e.target.id)
    );
  ...
  const setFeedback = (
    <Col span={12} style={{ marginTop: 20 }}>
      <Row style={{ textAlign: "right", marginBottom: 15 }}>
        <Button type="primary" size="large" onClick={PopupAddSubject}>
          <strong>주제 추가</strong>
        </Button>
      </Row>
      <Row>
  ...
  const getFeedback = (
    <Col span={12} style={{ marginTop: 20 }}>
      <Row>
        <Empty
          description={
            <div style={{ marginTop: 70, fontSize: 25 }}>
              <Text>
                <strong>내 피드백으로 전환해주세요~</strong>
              </Text>
            </div>
          }
        />
  ...
  return (
    <>
      ...
      <div>
        <AddSubject visible={visible} handleCancel={handleCancel} />
      </div>
      <div>
        <UpdateSubject
